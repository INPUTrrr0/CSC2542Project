...
from simple_rl.agents.func_approx.intra_dqn.DQN import DQNAgents
...
class Option(object):
    ...
    def __init__(self, overall_mdp, name, global_solver, lr, ddpg_batch_size, classifier_type="ocsvm",
                 subgoal_reward=0., max_steps=20000, seed=0, parent=None, num_subgoal_hits_required=3, buffer_length=20,
                 dense_reward=False, enable_timeout=True, timeout=200, initiation_period=2,
        ...
        self.writer = writer
        # self.lr = lr
        self.batchsize = ddpg_batch_size
        ...
        learn_rate=lr, target_tau=2e-3, update_rate=4, seed=0)
        self.solver = DQNAgents(solver_name,state_size, action_size, dqn_type='DQN', replay_memory_size=1e5, batch_size=self.batchsize, gamma=0.99,
        learn_rate=lr, target_tau=2e-3, update_rate=4, seed=0)
    ...
    def batched_is_init_true(self, state_matrix):
        ...
        # test = self.initiation_classifier.predict(position_matrix) == 1
        return self.initiation_classifier.predict(position_matrix) == 1

    def is_init_true(self, ground_state):
        if self.name == "global_option":
            return True
        features = ground_state.features() if isinstance(ground_state, State) else ground_state.data
    ...
    def is_term_true(self, ground_state):
        ...
        return self.overall_mdp.is_goal_state()
    ...
    def add_initiation_experience(self, states):
        ...
        segmented_positions = [segmented_state.features() for segmented_state in segmented_states]
    ...
    def train_one_class_svm(self):
        ...
        self.initiation_classifier = svm.OneClassSVM(kernel="rbf", nu=0.5, gamma="scale")  # TODO: TUNE NU
    ...
    def train_two_class_classifier(self):
        ...
        kwargs = {"C": 0.1, "kernel": "rbf", "gamma": "scale", "class_weight": "balanced"}
        # else:
    ...
    def update_option_solver(self, s, a, r, s_prime):
        """ Make on-policy updates to the current option's low-level DDPG (DQN) solver. """
        ...
        if self.is_term_true(s):
            print("[update_option_solver] Warning: called updater on {} term states:\n{}".format(self.name, s))
            return
    ...
    def execute_option_in_mdp(self, mdp, step_number):
        ...
        if self.is_init_true(state):
            ...
            is_term = self.is_term_true(state)
            is_state_term = state.is_terminal()
            while not is_term and not is_state_term and \
                    step_number < self.max_steps and num_steps < self.timeout:
                # print("state_features is ",state.features())
                if isDebug:
                    print(f"------- Current Option: {self.name} -------")
                action = self.solver.act(state.features())
                action = int(action)
            ...
            return option_transitions, total_reward, last_state
    ...
    def refine_initiation_set_classifier(self, visited_states, start_state, final_state, num_steps,
        ...
        if self.is_term_true(final_state):  # success
            ...
            negative_examples = [start_state.features()]
