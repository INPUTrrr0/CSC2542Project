...
class FeudalNetwork(nn.Module):
    def __init__(self,
        ...
        self.manager = Manager(self.c, self.d, args, device)
        self.worker = Worker(self.b, self.c, self.d, self.k, n_actions, device)
    ...
    def forward(self, x, goals, states, mask, save=True):
        ...
        goal, state, value_m = self.manager(z)
        ...
        action_dist, value_w = self.worker(z, goals[:self.c + 1], mask)
...
class Manager(nn.Module):
    def __init__(self, c, d, args, device):
        super().__init__()
...
class Worker(nn.Module):
    def __init__(self, b, c, d, k, num_actions, device):
        ...
        self.MLP = nn.Sequential(
                nn.Linear(self.d, 64),
                nn.ReLU(),
                nn.Linear(64, k * self.num_actions),
                nn.ReLU())
    ...
    def forward(self, z, goals, mask):
        ...
        a = torch.einsum("ij, ijk -> ik", w, u).softmax(dim=-1)
